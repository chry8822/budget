...
const barColors = ['#87C2CA', '#DF93B4', '#F1C764', '#47B39C', '#EC6B56', '#A855F7'];
...
export default function StatsScreen() {
  ...
  // 카테고리 데이터 정리
  const totalForPie = categoryRows.reduce((sum, r) => sum + (r.amount || 0), 0);

  const sortedCategoryRows = [...categoryRows].sort((a, b) => b.amount - a.amount);
  ...
  const paymentTotal = paymentRows.reduce((sum, r) => sum + (r.amount || 0), 0);
        ...
        >
          ...
          <View style={styles.headerRow}>
            <View style={styles.rangeTabs}>
              <Pressable
                style={[styles.rangeTab, range === 'thisMonth' && styles.rangeTabActive]}
                onPress={() => setRange('thisMonth')}
              >
                <Text
                  style={[styles.rangeTabText, range === 'thisMonth' && styles.rangeTabTextActive]}
                >
                  이번 달
                </Text>
              </Pressable>
              ...
              <Pressable
                style={[styles.rangeTab, range === 'lastMonth' && styles.rangeTabActive]}
                onPress={() => setRange('lastMonth')}
              >
                <Text
                  style={[styles.rangeTabText, range === 'lastMonth' && styles.rangeTabTextActive]}
                >
                  지난 달
                </Text>
              </Pressable>
              ...
              <Pressable
                style={[styles.rangeTab, range === 'custom' && styles.rangeTabActive]}
                onPress={() => {
                  // setRange('custom');
              ...
              >
                <Text
                  style={[styles.rangeTabText, range === 'custom' && styles.rangeTabTextActive]}
                >
                  선택
                </Text>
              </Pressable>
          ...
          <View style={styles.statsCardsWrap}>
            ...
            <View style={styles.card}>
              ...
              <Text style={styles.rowValueBold}>{formatWon(totalExpense)}</Text>
            </View>
            ...
            <View style={styles.card}>
              <View style={styles.cardHeaderRow}>
                ...
                {totalForPie > 0 && (
                  <Text style={styles.cardSubtitle}>총 {formatWon(totalForPie)}</Text>
                )}
              </View>
              ...
              {sortedCategoryRows.length === 0 ? (
                ...
                <View style={styles.pieRow}>
                  ...
                  <View style={styles.legendCol}>
                    {topRows.map((row, idx) => {
                      const percent =
                        totalForPie > 0 ? Math.round((row.amount / totalForPie) * 100) : 0;
                    ...
                    {othersTotal > 0 && (
                      <View style={styles.row}>
                        <View style={styles.rowLeft}>
                          <View style={[styles.colorDot, styles.colorDotOthers]} />
                          <Text style={styles.rowLabel}>기타</Text>
                        </View>
                        <View style={styles.rowRight}>
                          <Text style={styles.rowValue}>{formatWon(othersTotal)}</Text>
            ...
            <View style={styles.card}>
              <View style={styles.cardHeaderRow}>
                ...
                {paymentTotal > 0 && (
                  <Text style={styles.cardSubtitle}>총 {formatWon(paymentTotal)}</Text>
                )}
              </View>
