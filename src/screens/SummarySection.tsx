    ...
    <>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.subText}>
          ...
          달 남은 일수: <Text style={{ fontWeight: 'bold' }}>{remainingDays}일</Text>
        </Text>
        <Text style={styles.todayText}>
          <Ionicons name="alert-circle-outline" size={16} color={theme.colors.primary} /> 오늘{' '}
          <Text style={{ color: theme.colors.primary }}>{formatWon(todayExpense)}</Text> 썼어요
        </Text>
      </View>
        ...
        {!loading && last3.length > 0 && (
          <View style={styles.miniChartContainer}>
            <Text style={styles.miniChartTitle}>
              <Text>최근 3일 지출</Text>
              <Text style={{ fontSize: theme.typography.sizes.xs, color: theme.colors.textMuted }}>
                {' '}
                ({month}월)
              </Text>
            </Text>
